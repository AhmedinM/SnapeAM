...
if(isset($_SESSION["descriptionError"])){
    $desErr = $_SESSION["descriptionError"];
}

if(isset($_SESSION["ingredientError"])){
    $inErr = $_SESSION["ingredientError"];
}

if(isset($_SESSION["insertR"])){
    $insert = $_SESSION["insertR"];
    unset($_SESSION["insertR"]);
}
            ...
            <div style="background-color: white; width: 100%; min-height: 86%">
                ...
                <form method="post" action="insert_r.php"> 
                    <div class="col-12 ml-5 mr-n5">
                        <label for="potionInput" class="mt-3">Napitak:</label> <br>
                        <select class="bg-secondary text-success" name="potion" id="potionInput">
                            <?php
                                if($pot!==null){
                                    for($i=0;$i<count($pot);$i++){
                                        echo "<option value=\"".$pot[$i]["id"]."\">".$pot[$i]["name"]."</option>";
                                    }
                                }
                            ?>
                        </select>
                    ...
                    <div class="col-12 ml-5 mr-n5">
                        ...
                        <textarea class="bg-secondary text-success" name="description" id="descriptionInput" cols="30" rows="5"></textarea>
                        <?php if($desErr!==null){
                            echo "<div><span class=\"bg-white text-danger mt-3\">".$desErr."</span></div>";
                        } ?>
                    </div>
                   
                    <div class="col-12 ml-5 mr-n5">
                    <br>
                        <label for="">Sastojci:</label> <br>
                        <?php if($inErr!==null){
                            echo "<div><span class=\"bg-white text-danger mt-3\">".$inErr."</span></div>";
                        } ?>
                        <?php
                            if($ing!==null){
                                for($i=0;$i<count($ing);$i++){
                                    echo "<input type=\"checkbox\" name=\"ingredients[]\" value=\"".$ing[$i]["id"]."\"> ".$ing[$i]["name"]."<br>";
                                    echo "Količina: <input class=\"bg-secondary text-success\" type=\"number\" name=\"".$ing[$i]["id"]."\">g<br><br>";
                                }
                            }
                        ?>
                    </div>
                    <div class="col-12 ml-5 mr-n5 mt-3">
                        <button id="save" class="mt-3 btn btn-dark text-success"><i class="fa fa-save"></i> Sačuvaj</button>
                    </div>
                </form>
                <br><br>
            </div>
